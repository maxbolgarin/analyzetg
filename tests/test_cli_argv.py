...
The preprocessor exists so users can type `unread analyze -1003865481227`
without Click mistaking the chat_id for a short-option bundle. Earlier
it injected `--` in-place, which broke
`unread analyze -1003865481227 --all-flat` (the `--` closed option parsing
...
from unread.cli import _preprocess_argv


def test_no_negative_number_unchanged() -> None:
    assert _preprocess_argv(["unread", "analyze", "@foo"]) == [
        "unread",
        "analyze",
        "@foo",
    ]
...
def test_bare_negative_number_moves_to_end() -> None:
    ...
    assert _preprocess_argv(["unread", "analyze", "-1003865481227"]) == [
        "unread",
        "analyze",
        "--",
        "-1003865481227",
...
def test_negative_number_then_flag_regression() -> None:
    # The real-world regression: `unread analyze -1003865481227 --all-flat`.
    # Old behavior: `unread analyze -- -1003865481227 --all-flat` → Click
    ...
    assert _preprocess_argv(["unread", "analyze", "-1003865481227", "--all-flat"]) == [
        "unread",
        "analyze",
        "--all-flat",
        "--",
...
def test_flag_before_negative_number() -> None:
    # `unread analyze --all-flat -1003865481227` — negative id is still a
    ...
    result = _preprocess_argv(["unread", "analyze", "--all-flat", "-1003865481227"])
    # The negative number was NOT moved (prev was `--all-flat`).
    assert result == ["unread", "analyze", "--all-flat", "-1003865481227"]


def test_option_value_pattern_not_rewritten() -> None:
    # `unread backfill --chat -1001234 --from-msg 5000` — the negative
    # number is the value of `--chat`, not a positional. Must stay
    # next to `--chat` so Click reads it as the option's value.
    assert _preprocess_argv(["unread", "backfill", "--chat", "-1001234", "--from-msg", "5000"]) == [
        "unread",
        "backfill",
        "--chat",
        "-1001234",
...
def test_multiple_flags_before_and_after_id() -> None:
    # Real shape: `unread analyze -1003... --preset summary --last-days 7`.
    assert _preprocess_argv(
        ["unread", "analyze", "-1003865481227", "--preset", "summary", "--last-days", "7"]
    ) == [
        "unread",
        "analyze",
        "--preset",
...
def test_short_flag_not_confused_with_negative_number() -> None:
    ...
    # the rewrite.
    assert _preprocess_argv(["unread", "analyze", "@foo", "-c"]) == [
        "unread",
        "analyze",
        "@foo",
        "-c",
...
def test_negative_id_under_nested_subcommand() -> None:
    ...
    assert _preprocess_argv(["unread", "chats", "add", "-1001234", "--full-history"]) == [
        "unread",
        "chats",
        "add",
        "--full-history",
