...
class WebsiteCfg(_StrictCfg):
    """Knobs for `atg analyze <website-url>` page fetch + extraction.
...
def load_settings(config_path: Path | str | None = None) -> Settings:
    ...
    # `ATG_CONFIG_PATH` is the new name; `ANALYZETG_CONFIG_PATH` is honored as
    # a back-compat alias for users who set it before the package rename.
    cfg_path = Path(
        config_path
