...
from atg.analyzer.formatter import build_link_template, format_messages
from atg.analyzer.openai_client import chat_complete, make_client
from atg.ask.retrieval import retrieve_messages, tokenize_question
from atg.config import get_settings
from atg.core.paths import compute_window
from atg.db.repo import open_repo
from atg.i18n import t as _t
from atg.i18n import tf as _tf
from atg.tg.client import tg_client
from atg.tg.folders import list_folders, resolve_folder
from atg.tg.resolver import resolve as resolve_ref
...
async def cmd_ask(
    *,
    ...
    if _no_scope:
        from atg.interactive import run_interactive_ask
    ...
    async with tg_client(settings) as client, open_repo(settings.storage.data_path) as repo:
        ...
        if chat:
            ...
            if with_comments:
                from atg.tg.topics import get_linked_chat_id
        ...
        if chat_ids:
            from atg.analyzer.commands import build_enrich_opts as _build_enrich_opts
            from atg.enrich.pipeline import enrich_messages
        ...
        if build_index:
            from atg.ask.embeddings import build_index as _build_index
            from atg.ask.embeddings import default_model as _default_embed_model
        ...
        async def _maybe_mark_read() -> None:
            ...
            try:
                ...
                if not max_id:
                    return
                from atg.tg.dialogs import mark_as_read as _mark_as_read
...
async def _run_single_turn(
    *,
    ...
    """
    from atg.core.paths import derive_internal_id
    from atg.util.pricing import chat_cost
    ...
    if semantic:
        from atg.ask.embeddings import default_model as _embed_model_name
        from atg.ask.embeddings import semantic_search
    ...
    if rerank_on and len(scored) > min(limit, ask_cfg.rerank_keep):
        from atg.ask.rerank import rerank as _rerank_fn
...
async def _refresh_chats(
    ...
    *,
    ...
    from atg.tg.sync import backfill
