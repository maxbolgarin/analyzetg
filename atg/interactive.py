...
from atg.analyzer.prompts import Preset, get_presets
from atg.config import get_settings
from atg.db.repo import open_repo
from atg.i18n import t as i18n_t
from atg.i18n import tf as i18n_tf
from atg.tg.client import tg_client
from atg.tg.dialogs import list_unread_dialogs
...
async def run_interactive_analyze(
    *,
    ...
    """Default UX for `atg analyze` (no ref). Walk wizard, then run.
    ...
    if answers.run_on_all_unread:
        from atg.analyzer.commands import run_all_unread_analyze
...
async def run_interactive_dump(
    *,
    ...
    """Default UX for `atg dump` (no ref). Wizard without preset step."""
    answers = await _collect_answers(
        mode="dump",
        console_out=console_out,
        output=output,
    ...
    if answers.run_on_all_unread:
        from atg.export.commands import run_all_unread_dump
...
async def run_interactive_describe() -> None:
    """Default UX for `atg describe` (no ref, no filters): pick → show."""
...
def _estimate_cost(
    *,
    ...
    """
    from atg.analyzer.pipeline import estimate_cost as _est
...
async def _pick_chat(
    ...
    *,
    ...
    Returns one of:
      ...
      - None — cancelled
    """
    from atg.tg.folders import chat_folder_index
...
async def _pick_from_all(
    ...
    *,
    ...
    """
    from atg.tg.client import _chat_kind, entity_id, entity_title, entity_username
    from atg.tg.dialogs import UnreadDialog, correct_forum_unread
