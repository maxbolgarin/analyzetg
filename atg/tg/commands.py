...
from atg.tg.dialogs import get_unread_state
from atg.tg.links import parse
from atg.tg.resolver import resolve
from atg.tg.topics import (
...
async def cmd_doctor() -> None:
    ...
    env_path = cwd / ".env"
    cfg_path = _Path(
        os.environ.get("ATG_CONFIG_PATH") or os.environ.get("ANALYZETG_CONFIG_PATH") or "config.toml"
    )
    if env_path.exists():
    ...
    if db_path.exists():
        try:
            from atg.db.repo import open_repo as _open_repo
...
async def cmd_describe(
    ...
    *,
    ...
    if ref is None and not has_filters:
        from atg.interactive import run_interactive_describe
    ...
    *,
    ...
    async with tg_client(settings) as client, open_repo(settings.storage.data_path) as repo:
        ...
        if ref is None:
            ...
            from atg.interactive import _pick_chat as _picker
            ...
            from atg.interactive import BACK as _BACK
...
async def cmd_sync(chat: int | None, thread: int | None, dry_run: bool) -> None:
    from atg.tg.sync import sync_subscription
...
async def cmd_backfill(chat: int, from_msg: str, direction: str) -> None:
    from atg.tg.sync import backfill as run_backfill
...
async def cmd_chats_manage() -> None:
    ...
    from atg.interactive import LIST_STYLE, _expand_printable_for_search
