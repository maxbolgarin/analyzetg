"""OpenAI-embedding-backed retrieval for `atg ask --semantic`.

Index lifecycle:
  - `atg ask --build-index --chat <ref>` (or `--folder`) walks the local DB,
    finds messages without an embedding row, batches them, calls the
    embeddings API, and persists vectors.
  - `atg ask --semantic …` embeds the question, cosine-similarity-scans the
...
def _body_for_embedding(m: Message) -> str:
    ...
    Mirrors what `atg ask` retrieval scans: text + transcript. Image
