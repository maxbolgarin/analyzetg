...
async def cmd_prompt(
    *,
    ...
    """Send `prompt` straight to the active chat provider, return the answer text.
    ...
    the prompt. `--output` saves a markdown file; default is terminal
    render. `--console` forces terminal even when `--output` is set
    (mirrors `unread ask`).
    """
    ...
    if answer_lang:
        messages.append({"role": "system", "content": f"Respond in {answer_lang}."})
    messages.append({"role": "user", "content": prompt})

    # Cost preview against the actual messages list. Same shape as
    # ask/commands.py's preview block — same UX (estimate line, optional
    # --max-cost gate, --yes silences the confirm).
